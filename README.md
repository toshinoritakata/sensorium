...
### 機材取込: 任意URL（pnpm ingest:url）

`url-extract` アダプタ（ADR-0002）。公開APIの無いメーカー直販（Orbbec / Hokuyo 等）の製品ページを
1点ずつ fetch し、本文を Claude が envelope へ写像する。写像ロジックは DigiKey と共通（`scripts/ingest-lib.ts`）。

```sh
pnpm ingest:url "https://www.orbbec.com/products/tof-camera/femto-mega/"   # 既定 status:candidate
pnpm ingest:url "<URL>" "<URL>" --status active   # 信頼する定番だけ即有効化 → data/reference.seed.json
```

既定は `status:candidate`（候補ゲート）。`--status active` を明示したものだけ `data/reference.seed.json`
（`load-seeds` が読む active seed）へ入り、即評価対象になる。`spatial` 等の複雑カテゴリは structured output の
文法コンパイルがタイムアウトするため、写像は「JSON Schema を渡して JSON 出力 → zod 検証（不適合なら1回リトライ）」。
...
- **機材取込 url-extract（任意URL）** — メーカーページ fetch → Claude 写像（`pnpm ingest:url`）。定番は `--status active` で `data/reference.seed.json` へ。

後続: spatial の MountPlan 幾何（カメラ被覆 / FOV）、audio・touch の DetectionMethod 経路、
取込候補の人レビュー昇格フロー、flat-file アダプタ。
